...
- **Reversing a rule means DELETING the old one, in the same edit, everywhere it is stated** (2026-08-22). Writing the new rule and appending "since <date> this covers X, not Y" to the old one leaves a document that says both things, and the next session believes whichever it reads first. Grep for the OLD rule's words, not the new ones — one reversal about copy touched four files and six passages — and rewrite each in one pass. Same instinct as a tombstone comment left in code, and just as wrong.
