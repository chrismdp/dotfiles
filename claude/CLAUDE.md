...
- **Chris's only test address is `chris+<anything>@chrismdp.com`.** Never treat any other address as Chris's own, whatever the name looks like. An address found on a calendar event, in Kit, or in Gmail belongs to a real person, and a real person's address never goes on a test event, test email, or test message. `cp@chrismdp.com` is his private address and `chris@chrismdp.com` the public one; neither is for tests.
...
- **Delegate by default** (Chris, 2026-09-16). The coordinator's own usage is the scarce budget, because Chris mostly works with Fable and Astra class models. Any bounded coding, investigation or evaluation task that a cheaper worker can do goes to one, in its own worktree, following `/delegate`: Codex Sol or Astra first, then opus at xhigh, sonnet for mechanical work. The coordinator keeps design decisions, the brief, review of the returned diff, the affected gate, real model and browser checks, and release. **The main channel is for orchestration only** (Chris, 2026-09-17): the brief, the dispatch, the diff review, the affected gate, the merge, the release. Do the work yourself only when it is a UX call built interactively with Chris, a change so small that the brief would take longer than the change (one line, one value, one status flip), or a fix inside a review you are already doing. Three files by hand is a worker's job. Name the worker for each piece in the report.
